...
import random
import threading
from datetime import datetime, date             #importaciones de librerias que vamos a utilizar o que utilizaremos.
...
def generar_carnet_unico():
    ...
    prefijo = f"7691-{yy}-" #MANTIENE EL 7691 COMO EL CODIGO DE LA UNIVERSIDAD NO DE LA CARRERA 
    conn = get_db_connection()
    cursor = conn.cursor()
    try:
        while True:
            numero_random = random.randint(10000, 99999) #GENERA UN NUMERO ALEATORIO PARA QUE TENGA UN CARNET DIFERENTE CADA QUIEN
            carnet = f"{prefijo}{numero_random}"
            cursor.execute("SELECT id_persona FROM personas WHERE carnet = %s", (carnet,))
            if cursor.fetchone() is None:
                return carnet #VERIFICACAION QUE SI NO TIENE SIMILITUD QUE LA envie 
    finally:
        cursor.close()
        conn.close()
...
def generate_id_card_pdf(nombre, apellido, correo, foto_bytes, carnet, id_persona, firma_base64=None):
    ...
    try:
        ...
        pdf.add_page()
        pdf.set_auto_page_break(False)                                              #DISEÑO
        # Fondo blanco
        ...
        if not firma_bytes:
            try:
                ...
                row = cursor.fetchone()
                if row and row.get("firma"):                                         #DISEÑO
        ...
        qr_path = os.path.join("static", f"temp_qr_{id_persona}.png")                   #DISEÑO
        qr_img.save(qr_path)
...
def enviar_pdf_por_correo(destinatario, ruta_pdf, curso):
    remitente = "crisvalnit@gmail.com"
    password = "svtb bzdq zobt aynj"
